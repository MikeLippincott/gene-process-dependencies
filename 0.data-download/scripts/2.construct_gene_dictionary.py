...
entrez_genes = [
    x[1].strip(")").strip() for x in dependency_df.iloc[:, 1:].columns.str.split("(")
]

# Obtain intersection of entrez gene ids
entrez_intersection = list(set(gene_meta_df.entrezgene).intersection(set(entrez_genes)))
...
gene_passed_qc_df = (
    gene_meta_df.query("entrezgene in @entrez_intersection")
...
symbol_genes = [x[0].strip() for x in dependency_df.iloc[:, 1:].columns.str.split("(")]

gene_dictionary_df = pd.DataFrame(
    [entrez_genes, symbol_genes, dependency_df.iloc[:, 1:].columns.tolist()]
).transpose()
...
gene_dictionary_qc_df.loc[gene_dictionary_qc_df.qc_pass_pan != False, "qc_pass_pan"] = (
    True
)

# Create the qc_pass_other column
gene_dictionary_qc_df = gene_dictionary_qc_df.assign(
